...
def valid_match_id(api_client: BettingAPIClient) -> str:
    ...
    assert (
        matches
    ), "GET /api/matches returned no matches — cannot build a valid bet payload"
    return matches[0]["id"]
...
class TestStakeBoundaryValidation:
    ...
    def test_stake_above_maximum_is_rejected(
    ...
    @pytest.mark.api
    @pytest.mark.parametrize(
        "stake", [MIN_STAKE, MAX_STAKE], ids=["min_boundary", "max_boundary"]
    )
    def test_stake_at_valid_boundary_is_accepted(
        self, api_client, reset_balance, valid_match_id, stake
    ):
        response = api_client.place_bet(valid_match_id, "HOME", stake)
    ...
    @pytest.mark.api
    @pytest.mark.parametrize(
        "stake", [10.999, 5.001, 33.333], ids=["3dp_a", "3dp_b", "3dp_c"]
    )
    def test_stake_with_invalid_precision_is_rejected(
        self, api_client, reset_balance, valid_match_id, stake
    ):
        response = api_client.place_bet(valid_match_id, "HOME", stake)
        assert response.status_code == VALIDATION_ERROR_STATUS, (
...
class TestSelectionAndRequestValidation:
    ...
    @pytest.mark.api
    def test_invalid_selection_value_is_rejected(
        self, api_client, reset_balance, valid_match_id
    ):
        response = api_client.place_bet(
            valid_match_id, "HOME_WIN", 10.00
        )  # not HOME/DRAW/AWAY
        assert response.status_code == VALIDATION_ERROR_STATUS
    ...
    @pytest.mark.api
    def test_missing_stake_field_is_rejected(
        self, api_client, reset_balance, valid_match_id
    ):
        response = api_client.place_bet_raw(
            {"matchId": valid_match_id, "selection": "HOME"}
        )
...
class TestBalanceReset:
    ...
    def test_reset_balance_returns_to_starting_value_and_is_persisted(self, api_client):
        ...
        assert reset_response.json()["balance"] == pytest.approx(
            STARTING_BALANCE, abs=0.01
        )

        # Response body and persisted state must be consistent after reset (spec §5.3).
        balance_response = api_client.get_balance()
        assert balance_response.json()["balance"] == pytest.approx(
            STARTING_BALANCE, abs=0.01
        )
